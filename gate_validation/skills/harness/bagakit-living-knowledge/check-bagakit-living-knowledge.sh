...
if grep -q "$TMP_DIR/repo" <<<"$APPLY_OUT"; then
  echo "apply unexpectedly emitted absolute repo paths" >&2
  exit 1
fi
...
grep -q ".bagakit/.*host-local runtime state" docs/must-authority.md
...
sh "$CMD" ingest --root . --source docs/reviewed-note.md --dest notes/k-2ab7qxk9.md >/dev/null
test -f docs/notes/k-2ab7qxk9.md
...
cat > docs/howto-learning-20260426.md <<'EOF'
# Bad Timestamp Name

- This reviewed note still carries a capture-time filename.
EOF
if "$CMD" ingest --root . --source docs/howto-learning-20260426.md --dest notes/k-2ab7qxk9b.md >"$TMP_DIR/ingest-time-source.out" 2>"$TMP_DIR/ingest-time-source.err"; then
  echo "ingest unexpectedly accepted a timestamp-derived source markdown path" >&2
  exit 1
fi
grep -q "source markdown path must not encode wall-clock time" "$TMP_DIR/ingest-time-source.err"
rm docs/howto-learning-20260426.md

if "$CMD" ingest --root . --source docs/reviewed-note.md --dest notes/howto-learning-20260426.md >"$TMP_DIR/ingest-time-dest.out" 2>"$TMP_DIR/ingest-time-dest.err"; then
  echo "ingest unexpectedly accepted a timestamp-derived destination markdown path" >&2
  exit 1
fi
grep -q "destination shared markdown path must not encode wall-clock time" "$TMP_DIR/ingest-time-dest.err"

cat > docs/bad-meta.md <<'EOF'
---
source_path: docs/.bagakit/inbox/howto-learning-20260426.md
captured_at: 2026-04-26T08:30:00Z
---

# Bad Metadata

- This note still exposes raw source provenance.
EOF
if "$CMD" ingest --root . --source docs/bad-meta.md --dest notes/k-2ab7qxk9c.md >"$TMP_DIR/ingest-meta.out" 2>"$TMP_DIR/ingest-meta.err"; then
  echo "ingest unexpectedly accepted raw source-path or action-time metadata" >&2
  exit 1
fi
grep -q "raw source-path or action-time metadata keys" "$TMP_DIR/ingest-meta.err"
rm docs/bad-meta.md

mkdir -p docs/notes
cat > docs/notes/howto-learning-20260426.md <<'EOF'
# Timestamped Shared Page

- This file name should be rejected by doctor.
EOF
if "$CMD" doctor --root . >"$TMP_DIR/doctor-time-name.out" 2>"$TMP_DIR/doctor-time-name.err"; then
  echo "doctor unexpectedly accepted a timestamp-derived shared knowledge path" >&2
  exit 1
fi
grep -q "shared knowledge path must not encode wall-clock time" "$TMP_DIR/doctor-time-name.err"
rm docs/notes/howto-learning-20260426.md
sh "$CMD" doctor --root . >/dev/null
