...
PLANNING_ENTRY_JSON="$LOCAL_ROOT/.bagakit/planning-entry/handoffs/local-route.json"
EXPORT_OUTPUT="$(
  python3 "$SKILL_DIR/scripts/bagakit-brainstorm.py" export-planning-entry-handoff \
    --root "$LOCAL_ROOT" \
    --dir "$LOCAL_ARTIFACT" \
    --scene ambiguous_delivery \
    --recipe-id planning-entry-brainstorm-to-feature
)"
assert_output_line "$EXPORT_OUTPUT" "scene=ambiguous_delivery"
assert_output_line "$EXPORT_OUTPUT" "recipe_id=planning-entry-brainstorm-to-feature"
test -f "$PLANNING_ENTRY_JSON"
python3 - "$PLANNING_ENTRY_JSON" <<'PY'
import json
import sys
from pathlib import Path

payload = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
assert payload["schema"] == "bagakit/planning-entry-handoff/v1"
assert payload["status"] == "approved"
assert payload["producer_surface"] == "bagakit-brainstorm"
assert payload["recommended_route"]["scene"] == "ambiguous_delivery"
assert payload["recommended_route"]["recipe_id"] == "planning-entry-brainstorm-to-feature"
...
assert_contains_line "$STALE_CHECK_OUTPUT" "archive handoff action file missing:"
