...
export const SUITE: EvalSuiteDefinition = {
  ...
  summary: "Check deterministic grill run lifecycle, one-question progression, research-needed handoff, and generated brief projection.",
  ...
  cases: [
    {
      ...
      summary: "A grill run should progress through one question and one research-needed node while keeping the brief generated from structured truth.",
      focus: ["structured-ssot", "one-question", "research-needed", "readonly-brief"],
      run: (context) => {
        ...
        try {
          ...
          expectOk(runCommand("bash", [cli, "plan", "--root", tempRepo, "--run", "eval", "--node", "q001", "--question", "Is the target grillable?", "--decision", "intake boundary", "--recommended-answer", "Yes, it has a concrete target snapshot.", "--rationale", "Grill starts only after Spark-worthy vagueness is resolved."], { cwd: repoRoot, replacements }), "plan q001");
          ...
          expectOk(runCommand("bash", [cli, "plan", "--root", tempRepo, "--run", "eval", "--node", "r001", "--kind", "research_needed", "--depends-on", "q001", "--question", "What prior art should shape the next question?", "--decision", "question quality evidence", "--recommended-answer", "Use researcher through selector.", "--rationale", "Grill identifies research gaps without executing research."], { cwd: repoRoot, replacements }), "plan r001");
          ...
          const run = JSON.parse(fs.readFileSync(runPath, "utf8")) as { status: string; question_nodes: Array<{ status: string }>; qa_events: unknown[] };
          const brief = fs.readFileSync(briefPath, "utf8");
          assert.equal(run.status, "complete");
          assert.equal(run.question_nodes[0].status, "answered");
          assert.equal(run.question_nodes[1].status, "evidence_attached");
          assert.equal(run.qa_events.length, 1);
          ...
          return {
            ...
            commands: [
              ...
              `bash ${cli} plan --root <temp-repo> --run eval --node q001 ...`,
