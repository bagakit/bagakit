  ...
  cases: [
    {
      ...
      run: (context) => {
        ...
        try {
          ...
          const surveyResult = run(["candidate-survey", "--file", target, "--root", repoRoot, "--json"], "candidate-survey planning-entry");
          const rankingResult = run(["skill-ranking", "--file", target, "--json"], "skill-ranking planning-entry");
          run([
            "evaluate",
            "--file",
            target,
          ...
          const surveyPayload = JSON.parse(surveyResult.stdout) as {
            planned_candidates: Array<{ skill_id: string }>;
          };
          const rankingPayload = JSON.parse(rankingResult.stdout) as {
            rows: Array<{ skillId: string }>;
          };
          ...
          assert.ok(surveyPayload.planned_candidates.some((entry) => entry.skill_id === "bagakit-brainstorm"));
          assert.ok(surveyPayload.planned_candidates.some((entry) => entry.skill_id === "bagakit-feature-tracker"));
          assert.ok(surveyPayload.planned_candidates.some((entry) => entry.skill_id === "bagakit-flow-runner"));
          assert.ok(rankingPayload.rows.some((entry) => entry.skillId === "bagakit-brainstorm"));
          assert.ok(rankingPayload.rows.some((entry) => entry.skillId === "bagakit-feature-tracker"));
          assert.ok(rankingPayload.rows.some((entry) => entry.skillId === "bagakit-flow-runner"));
          ...
          return {
            ...
            commands: [
              ...
              `node --experimental-strip-types ${script} candidate-survey --file <temp-repo>/.bagakit/skill-selector/tasks/planning-entry/skill-usage.toml --root . --json`,
              `node --experimental-strip-types ${script} skill-ranking --file <temp-repo>/.bagakit/skill-selector/tasks/planning-entry/skill-usage.toml --json`,
            ],
            artifacts: [
    ...
    },
    {
      ...
      run: (context) => {
        ...
        const surveyResult = run(["candidate-survey", "--file", target, "--root", repoRoot, "--json"], "candidate-survey");
        run(["drivers", "--file", target, "--root", repoRoot, "--output", driverPack], "drivers");
        const rankingResult = run(["skill-ranking", "--file", target, "--json"], "skill-ranking");
        ...
        const surveyPayload = JSON.parse(surveyResult.stdout) as {
          summary: { project_preferences: number };
          planned_candidates: Array<{ skill_id: string; preference: string }>;
          project_preference_hints: Array<{ skill_id: string; preference: string }>;
        };
        const rankingPayload = JSON.parse(rankingResult.stdout) as {
          rows: Array<{ skillId: string; status: string }>;
          evolver_review_signals: Array<{ signal_id: string }>;
        };
        const usageDoc = readSkillUsageDoc(target);
        ...
        assert.equal(surveyPayload.summary.project_preferences, 1);
        assert.ok(surveyPayload.project_preference_hints.some((entry) => entry.skill_id === "bagakit-brainstorm" && entry.preference === "prefer"));
        assert.ok(surveyPayload.planned_candidates.some((entry) => entry.skill_id === "bagakit-researcher" && entry.preference === "neutral"));
        ...
        assert.ok(rankingPayload.rows.some((entry) => entry.skillId === "bagakit-skill-selector"));
        assert.ok(rankingPayload.evolver_review_signals.some((entry) => entry.signal_id === "driver-load-review"));
        assert.equal(evolverExportPayload.schema, "bagakit.evolver.signal.v1");
          ...
          commands: [
            `node --experimental-strip-types ${script} init --file <temp-repo>/.bagakit/skill-selector/tasks/demo/skill-usage.toml --task-id demo-task --objective "eval selector loop" --owner validator`,
              `node --experimental-strip-types ${script} candidate-survey --file <temp-repo>/.bagakit/skill-selector/tasks/demo/skill-usage.toml --root . --json`,
              ...
              `node --experimental-strip-types ${script} skill-ranking --file <temp-repo>/.bagakit/skill-selector/tasks/demo/skill-usage.toml --json`,
            ...
            outputs: {
              ranking_row_count: rankingPayload.rows.length,
    ...
    },
    {
      ...
      run: (context) => {
        ...
        try {
          ...
          const surveyPayload = JSON.parse(surveyResult.stdout) as {
            summary: { project_preferences: number };
            planned_candidates: Array<{ skill_id: string; visibility: string; availability: string; selected: boolean; preference: string }>;
          };
          assert.equal(surveyPayload.summary.project_preferences, 0);
          assert.ok(surveyPayload.planned_candidates.some((entry) => entry.skill_id === "bagakit-skill-selector" && entry.visibility === "repo_visible" && entry.availability === "available"));
          assert.ok(surveyPayload.planned_candidates.some((entry) => entry.skill_id === "external-tool" && entry.visibility === "task_declared" && entry.availability === "unknown" && entry.preference === "neutral"));
          ...
          return {
            ...
            commands: [
              `node --experimental-strip-types ${script} candidate-survey --file <temp-repo>/.bagakit/skill-selector/tasks/survey/skill-usage.toml --root . --preferences-file <temp-repo>/.bagakit/skill-selector/missing-preferences.toml --json`,
            ],
            artifacts: [{ label: "usage-log", path: target }],
            outputs: {
              planned_candidates: surveyPayload.planned_candidates.length,
            },
            replacements,
          };
        } finally {
