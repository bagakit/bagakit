...
Shared knowledge published through `living-knowledge` must stay low-leakage.

Required rules:

- shared pages, managed bootstrap text, examples, and generated guidance use
  repo-relative paths only
- absolute filesystem paths are forbidden in durable shared surfaces
- imported-source references must not encode wall-clock time, action
  timestamps, usernames, machine-local paths, or raw source file names in
  durable handles
- if a durable imported reference needs an id, use a short opaque repo-local
  token such as `k-2ab7qxk9`
- that token must stay opaque; it must not encode time, slugs, source names,
  source content, or host identity
- descriptive Bagakit-authored page names may remain descriptive; the ban is
  on raw source-derived or timestamp-derived naming leaking into durable shared
  knowledge
- raw source-path or action-time metadata such as `source_path`,
  `original_path`, `captured_at`, `created_at`, or `updated_at` must not be
  carried into shared knowledge pages
...
Reviewed ingestion must normalize raw capture names and metadata before shared
publication instead of preserving source-derived provenance strings.
