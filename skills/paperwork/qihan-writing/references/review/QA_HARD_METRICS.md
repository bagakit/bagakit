...
`qihan_write_lint.py` 不是只在公开发布前才跑。
...
- review / polish 后的准终稿
- 飞书稿、对外分享稿、长期沉淀稿
- 任何已经写成 markdown 文件、可以被脚本检查的 artifact
...
`WARN` / `FAIL` 默认阻塞终稿交付，除非用户明确接受偏离。`ADVISORY`
只提示 reviewer 关注，不单独阻塞。

## 0.5 大白话自检

去 AI 味之后，终稿交付前，必须做一次 plain-language pass。

检查问题：

- 这段能不能用大白话讲给没有当前聊天、repo 背景和内部术语的人听懂？
- 首屏是否已经说明对象、问题、判断和下一步？
- 术语第一次出现时，读者是否知道它在本文里承担什么角色？
- 关键判断能不能换成“谁做什么、什么改变、怎么验证”？
- 如果删掉隐喻或术语，剩下的 literal sentence 是否仍然成立？
...
- 是否已经做过大白话自检：无上下文读者能否用自己的话复述对象、核心判断和下一步
