...
## 结构
- 标题层级 ≤ 4（最多 ####）
- 长文至少有两层正文目录：H2 主梁 + H3 子结构。只用 H2 会让飞书目录读起来像平铺清单。
- 开头用 callout 给 TL;DR
- **分割线极简**：只用于“章节切换”，不要频繁用来做段落分隔（会显得像模板）。
- **标题命名**：标题后不加括号；用短语表达结论（例如“为什么这篇论文重要”而不是“论文重要性（XX）”）。对外文章的 H1 要有刊物感，H2/H3 要形成递进关系，不要写成孤立标签。
