...
- Use `python3 scripts/bagakit-brainstorm.py export-planning-entry-handoff --root <project-root> --dir <artifact-dir> --scene <scene> --recipe-id <recipe-id>` when one approved brainstorm artifact should become downstream planning-entry exchange json.
...
- Route choice remains explicit through `--scene` and `--recipe-id`; brainstorm
  does not silently invent selector routing.
