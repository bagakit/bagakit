...
- prefer `candidate-survey --json` when eval or validation wants shortlist
  semantics without scraping markdown
...
Its `--json` form is the preferred machine-readable proof surface when one
check wants ranking semantics instead of markdown presentation.
