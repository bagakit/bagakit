...
TIME_DERIVED_NAME_RE = re.compile(
    r"(?<!\d)(?:19|20)\d{2}(?:[-_]?(?:0[1-9]|1[0-2])(?:[-_]?(?:0[1-9]|[12]\d|3[01])))(?:[Tt_-]?(?:[01]\d|2[0-3])(?:[:_-]?[0-5]\d){0,2}Z?)?(?!\d)"
)
...
def time_derived_name_parts(rel: str) -> list[str]:
    parts: list[str] = []
    for part in Path(rel).parts:
        token = part[:-3] if part.endswith(".md") else part
        if token and TIME_DERIVED_NAME_RE.search(token):
            parts.append(part)
    return parts


def raw_source_metadata_keys(frontmatter: dict[str, Any]) -> list[str]:
    return [key for key in RAW_SOURCE_METADATA_KEYS if key in frontmatter]
...
def write_if_needed(root: Path, path: Path, text: str, force: bool) -> str:
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists():
        if path.read_text(encoding="utf-8") == text:
            return f"skip: {display_path(root, path)}"
        if not force:
            return f"skip: {display_path(root, path)} already exists"
    path.write_text(text, encoding="utf-8")
    return f"write: {display_path(root, path)}"


def write_generated(root: Path, path: Path, text: str) -> str:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def ensure_agents_block(root: Path, target: Path, config: KnowledgeConfig) -> str:
    ...
    if not target.exists():
        ...
        return f"write: {display_path(root, target)}"
    text = target.read_text(encoding="utf-8")
    if START_TAG in text and END_TAG in text:
        updated = re.sub(re.escape(START_TAG) + r".*?" + re.escape(END_TAG), block, text, flags=re.S)
        if updated != text:
            target.write_text(updated, encoding="utf-8")
            return f"update: {display_path(root, target)} (replaced managed block)"
        return f"skip: {display_path(root, target)} (managed block unchanged)"
    if text and not text.endswith("\n"):
        text += "\n"
    target.write_text(text + block + "\n", encoding="utf-8")
    return f"update: {display_path(root, target)} (appended managed block)"
...
def ensure_git_exclude(root: Path, generated_root: Path) -> str | None:
    ...
    if "# BAGAKIT:LIVING-KNOWLEDGE:START" in text and "# BAGAKIT:LIVING-KNOWLEDGE:END" in text:
        ...
        if updated != text:
            ...
            return f"update: {display_path(root, exclude)} (living-knowledge exclude block)"
        return f"skip: {display_path(root, exclude)} (living-knowledge exclude block unchanged)"
    if text and not text.endswith("\n"):
        text += "\n"
    exclude.write_text(text + block, encoding="utf-8")
    return f"update: {display_path(root, exclude)} (appended living-knowledge exclude block)"
...
def sanitize_source_content(root: Path, text: str) -> str:
    ...
    leaked_keys = raw_source_metadata_keys(frontmatter)
    if leaked_keys:
        raise SystemExit(
            "error: shared knowledge must not absorb raw source-path or action-time metadata keys: "
            + ", ".join(leaked_keys)
        )
    return text


def assert_nonleaky_shared_path(rel: str, *, label: str) -> None:
    leaked_parts = time_derived_name_parts(rel)
    if leaked_parts:
        raise SystemExit(
            f"error: {label} must not encode wall-clock time or action timestamps in durable knowledge paths: {rel}"
        )


def shared_page_hygiene_errors(root: Path, path: Path, text: str) -> list[str]:
    rel = relpath(root, path)
    issues: list[str] = []
    if root.as_posix() in text or REPO_ABSOLUTE_PATH_RE.search(text):
        issues.append(f"shared knowledge page must not contain absolute filesystem path literals: {rel}")
    leaked_parts = time_derived_name_parts(rel)
    if leaked_parts:
        issues.append(f"shared knowledge path must not encode wall-clock time or action timestamps: {rel}")
    frontmatter, _ = parse_frontmatter(text)
    leaked_keys = raw_source_metadata_keys(frontmatter)
    if leaked_keys:
        issues.append(
            "shared knowledge page must not record raw source-path or action-time metadata keys: "
            f"{rel} ({', '.join(leaked_keys)})"
        )
    return issues
...
def apply_command(args: argparse.Namespace) -> int:
    ...
    outputs.append(ensure_agents_block(root, s.agents, cfg))
    outputs.append(write_if_needed(root, s.generated_root / ".gitignore", "*\n!.gitignore\n", args.force))
    ...
    outputs.append(write_if_needed(root, s.must_recall, render_template("must-recall-template.md"), args.force))
    outputs.append(
        write_if_needed(root, s.must_guidebook, build_guidebook(root, cfg, s.shared_root, s.system_root), args.force)
    )
    outputs.append(
        write_if_needed(
            root,
    ...
    outputs.append(write_generated(root, s.must_sop, build_must_sop(root, s.shared_root, s.system_root)))
...
def ingest_command(args: argparse.Namespace) -> int:
    ...
    source_rel = relpath(root, source)
    assert_nonleaky_shared_path(source_rel, label="source markdown path")
    ...
    dest_rel = relpath(root, dest)
    assert_nonleaky_shared_path(dest_rel, label="destination shared markdown path")
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_text(sanitize_source_content(root, source.read_text(encoding="utf-8")), encoding="utf-8")
    print(f"ingested: {source_rel} -> {dest_rel}")
    return 0
...
def doctor_command(args: argparse.Namespace) -> int:
    ...
    if s.agents.is_file():
        ...
        if root.as_posix() in agents_text or REPO_ABSOLUTE_PATH_RE.search(agents_text):
            errors.append("AGENTS.md must not contain absolute filesystem path literals")
    ...
    for path in iter_markdown(root, s.shared_root):
        ...
        errors.extend(shared_page_hygiene_errors(root, path, text))
    ...
    for path in nested_agents:
        ...
        text = path.read_text(encoding="utf-8", errors="replace")
        if root.as_posix() in text or REPO_ABSOLUTE_PATH_RE.search(text):
            errors.append(f"path-local AGENTS.md must not contain absolute filesystem path literals: {relpath(root, path)}")
        error = nested_agents_error(path, root, text)
